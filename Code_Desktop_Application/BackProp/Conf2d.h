...
public:
	...
	~Conv2d() {
		...
		if (mLayerOutput != nullptr){
			delete[] mLayerOutput; mLayerOutput = nullptr;
		}
						...
						for (int ky = 0; ky < K[0]; ++ky) {
							for (int kx = 0; kx < K[1]; ++kx) {
								...
								if (iy >= 0 && iy < H && ix >= 0 && ix < W) {
									// Indices für Input, Gewicht, Output
