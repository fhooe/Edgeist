...
public:
	...
	BatchNorm1d(model<T>* m, void* HeaderPointer, void* DataPointer, OptimizerID OptimizerType) : Layer<T>(m), mPtrLayer(HeaderPointer), mPtrData(DataPointer), mOptimizerType(OptimizerType) {
	...
	~BatchNorm1d() {
		...
		delete mRunningMean; mRunningMean = nullptr;
		delete mRunningVar; mRunningVar = nullptr;
	}
	...
	virtual ErrorType backwardPass(const T* input_data, T* output_data) override {
		...
		// Aufr�umen
		delete[] dL_dgamma;dL_dgamma = nullptr;
		delete[] dL_dbeta;dL_dbeta = nullptr;
		delete[] dL_dnorm;dL_dnorm = nullptr;
	...
	// L�sche Mittlwert und varianz nach minibatch
