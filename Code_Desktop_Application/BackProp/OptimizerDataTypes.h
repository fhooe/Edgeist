...
public:
	...
	~OptimizerSGD() {
		if (this->mData != nullptr) {
			delete[] this->mData; this->mData = nullptr;
		}

	}

	inline T getData(uint32_t index) const {
		if (index >= mData_size)
		{
			return T(0);
		}
	...
	inline T getM(uint32_t index) const {
		return T(0);
	}

	inline T getV(uint32_t index) const {
		return T(0);
	}
	...
	virtual ErrorType update(uint32_t index, T gradient, T learningRate, uint32_t timestep = 1) override {
		if (index >= mData_size) return ErrorType::IndexOutOfBounds;
