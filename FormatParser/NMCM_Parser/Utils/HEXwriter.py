            ...
            for layer in model_struct:
                if layer.hex_trainable != b'':
                    ...
                    if layer.json_data["Weights_amount_trainable"] != 0:
                        ...
                        offset_data = HexConverter(offset + layer.json_data["Weights_trainable_offset"], offset_table_type[1])
                        offset_pos = layer.Offset_Tabel_pos + pos * Sizeof(offset_table_type[1])
                        layer.hex_data = layer.hex_data[:layer.Offset_Table_pos + offset_pos] + offset_data + layer.hex_data[layer.Offset_Table_pos + offset_pos + len(offset_data):]
                        layer.json_data["Weights_trainable_offset"] = offset + layer.json_data["Weights_trainable_offset"]

                    if layer.json_data["Bias_amount_trainable"] != 0:
                        pos = list(layer.dataorder.keys()).index("Bias_trainable")
                        offset_data = HexConverter(offset + layer.json_data["Bias_trainable_offset"], offset_table_type[1])
                        offset_pos = layer.Offset_Tabel_pos + pos * Sizeof(offset_table_type[1])
                        layer.hex_data = layer.hex_data[:layer.Offset_Table_pos + offset_pos] + offset_data + layer.hex_data[layer.Offset_Table_pos + offset_pos + len(offset_data):]
