...
class Header():
    ...
    def define_data(self, model_stuct):
        ...
        self.data["DimensionInput_x"] = 1 if not ("DimensionInput_x" in model_stuct[0].data) else model_stuct[0].data["DimensionInput_x"]
        self.data["DimensionInput_y"] = 1 if not ("DimensionInput_y" in model_stuct[0].data) else model_stuct[0].data["DimensionInput_y"]
        self.data["ChannelsOut"] = 1 if not ("ChannelsOut" in model_stuct[-1].data) else model_stuct[-1].data["ChannelsOut"]
        self.data["DimensionOutput_x"]= 1 if not ("DimensionOutput_x" in model_stuct[-1].data) else model_stuct[-1].data["DimensionOutput_x"]
        self.data["DimensionOutput_y"]= 1 if not ("DimensionOutput_y" in model_stuct[-1].data) else model_stuct[-1].data["DimensionOutput_y"]
