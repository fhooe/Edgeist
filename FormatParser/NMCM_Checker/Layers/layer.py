...
class Layer (ABC):
    ...
    def _read_mask(self, data, name, length, data_read):
        bit_to_byte = 8
        data, data_read = self.aligne(data, data_read, Sizeof("uint32_t"))
    ...
    def _read_data(self, data, name, pos, data_read):
        ...
        if self.json_data["Offset_Table"][pos] != 0:
            ...
            data, data_read = self.aligne(data, data_read, datasize)
            if self.json_data["Offset_Table"][pos+1] != 0:
                # split
                trainable_weights = (self.json_data["Offset_Table"][pos+1] - self.json_data["Offset_Table"][pos]) // datasize
                for idx in range(0, trainable_weights):
                    current_data = data[:datasize]
                    # get float value from hex string
                    self.json_data[name + "_trainable"].append(BinaryConverter(current_data,"float32_t"))
                    data = data[datasize:]
                    data_read += datasize
        ...
        if self.json_data["Offset_Table"][pos] != 0:
            ...
            for idx in range(0, used):
                current_data = data[:datasize]
                # get float value from hex string
                self.json_data[name + "_frozen"].append(BinaryConverter(current_data,"float32_t"))
                data = data[datasize:]
                data_read += datasize
        
        return data, pos, data_read
    
    def aligne(self, data, data_read, alignement):
        offset = alignement - data_read % alignement

        if offset != alignement:
            data = data[offset:]
            data_read += offset
